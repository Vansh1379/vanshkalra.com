...
const TechComponent = () => {
  return (
    <div className="h-[155vh] sm:h-screen relative w-full overflow-hidden bg-black flex flex-col items-center justify-center rounded-lg">
      ...
      <h1
        className={cn(
          "text-4xl text-white z-10 md:text-6xl font-bold text-center absolute top-10 mt-10"
        )}
      >
        Tech Stack
      </h1>
      <div className="flex flex-row flex-wrap absolute top-44  md:static justify-center gap-10 z-10">
        {technologies.map((technology) => (
          <div key={technology.name}>
            ...
            <div
              className="sm:hidden h-28 w-28 rounded-full gap-10 bg-gray-200 p-4 flex items-center justify-center"
              key={technology.name}
            >
